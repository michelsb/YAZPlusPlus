...
void YazSender::run()
{
    ...
    try
    {
        ...
        do // until doomsday
        {
            ...
            float target_resolution = m_resolution;
            bool first_sample = true; // FA extension
            float new_factor = 0.0; // FA extension
            float old_factor = 0.0; // FA extension
            float diff_factor = 0.0; // FA extension
            ...
            bool done = false;
            bool fa_detecting = true; // FA extension
            float old_remote_pcap_mean = 0.0; // FA extension
            ...
            while (!done && local_crawl)
            {
                ...
                float resol_spc = (m_curr_pkt_size * 8.0) / (curr_rate - target_resolution) * 1000000 - mb.m_local_pcap_mean;
                float maxdiff = std::max(1.0f, resol_spc);
                float diff = fabs(mb.m_remote_pcap_mean - mb.m_local_pcap_mean);
                bool compexp =  (diff > maxdiff);
                ...
                if (compexp)
                {
                    ...
                    if (m_verbose)
                        std::cout << "debug1," << runnum << "," << m_curr_pkt_size << "," << target_resolution << "," << mb.m_local_pcap_mean << "," << mb.m_remote_pcap_mean << std::endl;
                    ...
                    {
                        inc = diff/2;
                        m_target_spacing = int(mb.m_local_pcap_mean + inc); 

                        // FA extension - Michel Bonfim   
                        if (fa_detecting) {
                            old_factor = new_factor;
                            if (first_sample) {                                
                                new_factor = inc/mb.m_local_pcap_mean;     
                                first_sample = false;
                            } else {
                                if (old_remote_pcap_mean >= mb.m_remote_pcap_mean) { // Detecting anomalies caused by buffering
                                   fa_detecting = false;
                                } else {
                                    new_factor = (0.3*(inc/mb.m_local_pcap_mean))+(0.7*old_factor);
                                    diff_factor = fabs(old_factor-new_factor);
                                    if (diff_factor <= 0.15) {                        
                                        current_estimate = (float(m_curr_pkt_size) * 8.0 ) / (mb.m_local_pcap_mean / 1000000.0);
                                        done = true;
                                        if (m_verbose)
                                            std::cout << "## done. 802.11n frame aggregation detected. setting current estimate to " << current_estimate/1000.0 << std::endl;                             
                                    } /*else {
                                        if (diff_factor >= 0.35) //Detecting non-fa scenarios or fa with high cross-traffic
                                            fa_detecting = false;                                        
                                    }*/
                                }
                            }
                            old_remote_pcap_mean = mb.m_remote_pcap_mean;
                        }                                                
                    }  

                    // FA extension
                    if (m_verbose) {
                        std::cout << "old factor: " << old_factor << std::endl;
                        std::cout << "new factor: " << new_factor << std::endl;
                        std::cout << "|new factor - old factor|: " << diff_factor << std::endl;
                        std::cout << "new target spacing: " << m_target_spacing << std::endl;
                    }
                ...
                {   
                    ...
                    if (m_verbose)
                        std::cout << "debug1," << runnum << "," << m_curr_pkt_size << "," << target_resolution << "," << mb.m_local_pcap_mean << "," << mb.m_remote_pcap_mean << "," << current_estimate/1000.0 << std::endl;
